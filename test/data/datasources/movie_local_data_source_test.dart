...
void main() {
  ...
  group('save tv series watchlist', () {
    test('should return success message when insert to database is success',
            () async {
          // arrange
          when(mockDatabaseHelper.insertTvSeriesWatchList(testMovieTable))
              .thenAnswer((_) async => 1);
          // act
          final result = await dataSource.insertTvSeriesWatchList(testMovieTable);
          // assert
          expect(result, 'Added to Watchlist');
        });
    ...
    test('should throw DatabaseException when insert to database is failed',
          ...
          when(mockDatabaseHelper.insertTvSeriesWatchList(testMovieTable))
              .thenThrow(Exception());
          // act
          final call = dataSource.insertTvSeriesWatchList(testMovieTable);
          // assert
          expect(() => call, throwsA(isA<DatabaseException>()));
        });
  });
